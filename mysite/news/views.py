...
def test(request):
    objects = ['j1', 'j2', 'j3', 'j4', 'j5', 'j6', 'j8', 'j9']
...
# def get_category(request, category_id): рефакторинг в класс отображения NewsByCategory
#     news = News.objects.filter(category_id=category_id, )
#     category = Category.objects.get(pk=category_id)
#     return render(request, 'news/category.html', {'news': news, 'category': category})


# def view_news(request, news_id): рефакторинг в класс отображения ViewNews
#     # news_item = News.objects.get(pk=news_id)
#     news_item = get_object_or_404(News, pk=news_id)
#     return render(request, 'news/view_news.html', {"news_item": news_item})


# def add_news(request): рефакторинг в класс отображения CreateNews
